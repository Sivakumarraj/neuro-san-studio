...
It is an agent-network implementation of **Evolutionary Surrogate-assisted Prescription (ESP)**: fit a
predictor from historical data, evolve decision policies against that predictor, and return a set of
strategies where no option beats every other on everything at once. The user picks from the set,
because that choice encodes what the objectives are worth to them — and nothing in the mathematics
knows that.
